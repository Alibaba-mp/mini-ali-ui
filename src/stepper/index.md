...
* `onChange` 在 [1.0.11](https://www.npmjs.com/package/mini-ali-ui?activeTab=versions) 版本中新增一个 `mode` 的返回值，用于判断用户是通过**点击**（`click`）还是通过**输入**（`input`）的方式改变值，两者之外就是 `undefined`；
